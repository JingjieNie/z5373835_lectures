...
# 1.1.2 Series.loc: Selection using sequence of labels will return a series
...
# When omitting column labels, pandas will return a series if the row label exists. Otherwise it will raise an exception
...
# All labels must exist. The following will raise a KeyError because a label is not part of ser.index
...
# (2) If either `start_label` or `end_label` not included in the index, the result will depend on
...
# Next, select a slice from 'a' to 'z'. Note that 'z' is not part of the index.
# Since the index is not sorted, the following will result in an error
# x = new_ser['b':'z']

# Series also have a method called `sort_index`, which will return a copy of the series with sorted indexes:
...
# Note that the label is case sensitive. For instance the following raises KeyError
# x = df['CLOSE']
...
# df[list of column labels] --> dataframe with columns in the same order as the column labels
...
# `x` will be an empty datafame because the slice is not part of the row labels
# x = df['Close': 'Bday']
# print(x)
