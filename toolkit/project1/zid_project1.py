...
def get_tics(pth):
    ...
    for line in lines:
        ...
        exchange, tic = line.split('=')
        tic = "".join(filter(str.isalpha, tic))
        exchange = "".join(filter(str.isalpha, exchange))
        tic_exchange_dic[tic] = exchange
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    filename = os.path.join(DATDIR, f"{tic}_prc.dat")
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        return
    elif not tickers_lst:
        raise Exception("Tickers_lst is an empty list.")
    else:
        # Check if all tickers in tickers_lst are keys of tic_exchange_dic
        invalid_tic = []
        for t in tickers_lst:
            if t not in tic_exchange_dic:
                invalid_tic.append(t)
                raise Exception(f"Invalid tickers: {invalid_tic}")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is None:
        ...
        for t in col_lst:
            if t not in COLUMNS:
                invalid_col.append(t)
                raise Exception(f"Invalid columns: {invalid_col}")
    ...
    for tic in tickers_lst:
        ...
        for line in lines:
            ...
            for col_name in col_lst:
                temp_dict = {col_name: line_dict[col_name]}
                line_data.update(temp_dict)
            data_lst.append(line_data)
        tic_data = {'exchange': exchange, 'data': data_lst}
        line_data_dict = {tic: tic_data}
        data_dict.update(line_data_dict)
    return data_dict
